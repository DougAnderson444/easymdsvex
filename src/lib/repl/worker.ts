...
// import json from 'rollup-plugin-json' // cant import outside a module?
...
// import { minify } from "terser"; // cant import outside a module?
...
function compare_to_version(major: number, minor: number, patch: number) {
	// @ts-ignore
...
async function assertModule(url: string): Promise<boolean> {
...
self.addEventListener('message', async (event: MessageEvent<Component[]>): Promise<void> => {
		...
		plugins: [
			{
				...
				async resolveId(importee: string, importer: string) {
					...
					try {
						...
						console.log({ fetched });
						console.error(error, { importee }, { importer });
					}
					...
					if (pkg && (pkg.svelte || pkg.browser || pkg.module || pkg.main)) {
						...
						const retUrl = (await assertModule(`${url}/index.js`))
							? `${url}/index.mjs`
							: `${url}/index.js`;
				...
				async transform(code: string, id: string) {
					...
					return preprocessPromise.then(({ code: v }) => {
						...
						(result.warnings || result.stats.warnings).forEach((warning: Warning) => {
	...
	let output: string = (await bundle.generate({ format: 'esm' })).output[0].code;

	// output = (await minify(output)).code; //, { sourceMap: true }

	self.postMessage({ output, warnings, diagnostics });
	// console.log({ ports: event.ports[0].postMessage });
	event.ports[0].postMessage({ output }); // reply on the given channel
});
